...
file_paths <- expand.grid(seed = seeds, fop = fop_proportion, pref_val = pref_val, ribo_speed = ribo_speed, trna_count = trna_count) %>%
  ...
  mutate(file_name = sprintf("trna_phage_pref%.1f_%d_fop%.1f_ribospeed%.6f_trna%d.tsv", pref_val, seed, fop, ribo_speed, trna_count)) %>%
...
dynamic_bar_sim_plot <- ggplot(dynamic_data, aes(x=factor(gene_number, gene_labels), 
  ...
  labs(x="gene", y="simulated proteins", title=sprintf("ribosome speed = %.6f     trna count = %d     time = %d", ribo_speed, trna_count, max_time)) +
  theme(axis.text.x=element_blank(), axis.ticks.x=element_blank()) +
  scale_y_continuous(expand = c(0,0)) +
  scale_fill_manual(values=cbPalette)

if (SAVE_FIGS) {
  save_plot(sprintf("analysis/figures/revised_protein_counts_dynamic_fop%.1f_pref%.1f_charge%d_ribospeed%.6f_trna%d.pdf", fop_proportion, pref_val, charge_val, ribo_speed, trna_count), dynamic_bar_sim_plot, base_width = 8, base_height = 7)
}
```