...
multiplier_vals <- c(0.001, 0.01, seq(0.1, 1.0, by = 0.1), 10.0, 100.0)
...
sim_data <- sim_data %>%
  mutate(
    multiplier =  as.numeric(str_extract(file, "(?<=weight_factor)[0-9]{1,2}\\.[0-9]+(?=_seed)")),
...
# Calculate average proteins for each multiplier at max time
sims <- gene_data %>%
...
if (SAVE_FIGS) {
  save_plot(sprintf("analysis/figures/fixed_gene%sproteins_by_weight_factor_%ds.pdf", gene_name, max_time), plot, base_width = 6, base_height = 6)
}
```