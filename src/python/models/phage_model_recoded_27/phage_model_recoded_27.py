...
#-------------------------------------------------------------------------------
    ...
    output_dir = "../../../../data/simulation/phage/parallel_output"
    output_filename = os.path.join(output_dir, f"output_{multiplier}.tsv")

    # Change the multiplier to 1, .9, .8, .7, .6, .5, .4, .3, .2, .1
    weights[gene10_start:gene10_stop] = [multiplier] * (gene10_stop - gene10_start)
#-------------------------------------------------------------------------------
    ...
    sim.simulate(time_limit=1000, time_step=5,
                 #output="phage_model_recoded_27.tsv")
